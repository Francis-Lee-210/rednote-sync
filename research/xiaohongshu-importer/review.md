...
- 阶段三继续完全离线，本插件的 `requestUrl`、`fetch` 和 vault 写入不能进入 Core。未来更合理的落点是独立 Obsidian projector/exporter：只消费已经校验、内容寻址并具逐资产 receipt 的 canonical 数据；在线 Provider 与知识库写入不得耦合在同一函数。
...
- 对照基线是本地 [`sync-core.md`](../../projects/rednote-sync-core/docs/sync-core.md)；候选插件不改变其中阶段三已完成、阶段四前暂停的边界。
...
- **checkpoint/恢复：缺失。** 网络、解析、媒体、Markdown和设置写入必须拆分；远端预取在未来Stage 4事务外执行，短事务重新校验expected account/revision/object refs，知识库projection失败不改变canonical正确性。
...
- **阶段边界：** 阶段三完全离线，不引入`requestUrl`、`fetch`或Obsidian runtime。未来即使采用Obsidian exporter，也应与Stage 4 Provider分离，不让在线HTML直接写知识库。
