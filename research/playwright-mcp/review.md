...
- 真实浏览器、persistent profile、CDP、extension、slow typing、延迟或隐藏自动化标志都不是风控有效性或账号安全证明。阶段三继续完全离线；未来阶段四若获得明确授权，也应实现窄化只读Adapter，而不是把通用Playwright MCP直接暴露给同步任务。
...
| Rednote cursor/assets | 项目无domain model | 完全由Stage 3 Core继续独占 |
...
- 不以通用Playwright MCP作为Stage 4 Provider；只实现`listPage/getDetail/getMediaDescriptor`窄接口。
...
- **Stage 3不改变：** 继续完全离线；本仓库、Playwright、浏览器、Cookie、CDP和MCP服务均不进入Core。
- **未来Stage 4另立规格和授权门：** Playwright MCP最多作为研究/诊断工具；生产Adapter应使用更窄的受审浏览器库/sidecar，具固定版本、专用profile、只读allowlist、SecretRef、审计和人工停止/恢复。
...
- **供应链、许可证与适配复审：PASS。** npm lock、CI/release、Docker、类型依赖、Apache-2.0 条件义务、B/D 分级及 Stage 3/4 边界均已复核。
