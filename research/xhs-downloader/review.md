...
当前建议保持 **A：值得作为阶段四 Adapter、媒体资产模型和恢复语义的专项参考，但只借鉴行为或独立重写，不直接嵌入源码**。主要原因是：API/MCP 服务模式启动后默认监听所有接口且没有显式应用层鉴权、Cookie 和动态 URL 缺少统一 secret 边界、请求客户端关闭 TLS 验证、供应链发布路径没有真正消费完整 lock，以及 GPLv3 文件与 README 附加措辞存在未解决的不一致。
...
- `[未知]` 自动滚动和页面当前已加载数组不能证明完整枚举、稳定排序或账号安全；这些事实仍需未来阶段四在明确授权下低频验证。
...
- `[采用边界]` 阶段四若参考其能力拆分，服务必须默认 loopback/IPC并强制认证；发现、平台读取和本地写为独立 capability；TLS 不可关闭；每次重定向验证媒体 host；输出路径做 resolved-root containment；Cookie、signed URL 和 token 不进入 CLI、配置、响应、日志或 trace。
...
| 阶段边界 | 不改变阶段三 | 所有网络、浏览器、Cookie 和动态在线事实仍只属于未来阶段四 |
...
- **供应链、许可证与项目适配复审：PASS。** 依赖计数、lock 边界、CI 权限、制品未知项和 GPL/README 分歧均有充分限定；A 级仅表示专项参考，阶段三继续离线。
