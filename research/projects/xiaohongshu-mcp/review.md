...
- 历史实施 Stage 3 继续完全离线。历史实施 Stage 4 若获明确授权，只能另建默认只读 Provider、强本地认证、专用 profile/账号绑定、URL/路径 allowlist、SecretRef 和全局停止闸；CDP、指纹 seed、固定延迟或“人性化”操作均不是反风控证明。
...
- `[适配判断]` 可借鉴 transport/service/action/model 分层，但当前 action 与浏览器、账号、DOM 和页面私有状态紧耦合，不能充当 历史实施 Stage 3 Core Provider。
...
本报告不把作者 README 的稳定性、频率或“降低风险”经验陈述当作平台规则或实证。任何真实账号实验都需要历史实施 Stage 4 的单独授权、最小请求预算和立即停止条件。
...
- 历史实施 Stage 4 Provider 只输出版本化、安全 DTO；MCP TextContent/raw `any` 不能直接进入 Core。
...
- **历史实施 Stage 3：不改变。** 继续完全离线；本仓库、浏览器、Cookie、token、MCP/REST 和在线 Action 均不进入 Core。
- **历史实施 Stage 4：另立授权门。** 即使采用 Playwright/Rod，也必须先完成平台授权、只读 API 规格、profile/account ownership、secret handling、停止/恢复和运行验收；浏览器“人性化”行为不改变合规结论。
