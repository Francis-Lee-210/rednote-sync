...
- `[静态证据，2026-08-16]` 具体核实 `Cialle/RedCrack`：仓库真实存在（MIT），自述“小红书 Web 端全站所有加密参数 Python 纯算逆向”，覆盖 `a1/webId/acw_tc/web_session`（README 标注“游客生成”）`/sec_poison_id/websectiga/gid` 与 `x-s/x-t/x-b3-traceid/x-xray-traceid` 签名头，同时暴露 `follow_user` 等平台写接口，强制代理，demo 显示登录态需外部提供 `web_session`。**状态变更（用户 2026-08-16 定向）**：由“仅记录存在、不采用”改为“候选参考，暂定服务于购买 Cookie 路线”；使用前仍需解决只读裁剪、无节流/停止条件、供应链与许可证复核问题。该变更与 `sync-core.md` §2.2 及 `catalog.md` 5.8 的既有排除面冲突，正式边界修订决策记录尚未创建，详见 `research/topics/one-time-cookie.md` §8。
...
目标：用低价值小号回答“裸 note_id 是否可用”和“跨账号 token 是否可复用”，主账号不参与在线探测。
...
2. 用户定向决定（C1–C6、E2）与一次性 Cookie 路线的完整研究（交付形态、三种接入方式、14 字段样本脱敏判定、风险与映射）记录于 `research/topics/one-time-cookie.md`，本文件不重复。要点：C1 购买一次性 Cookie 列为可选功能实现；C2 交付形态待商家信息；C3/C4 残余风险与合规风险接受；C5 是否严格只读待答复；C6 购买 Cookie 路线暂缓硬停止并尝试风控机制、自带小号 Cookie 保留硬停止；E2 RedCrack 改为候选参考（服务购买 Cookie 路线）。正式边界修订决策记录（`docs/decisions/2026-08-16-stage4-risk-isolation.md`）尚未创建，旧文档尚未修改。
3. 未改变的事实：裸 `note_id` 直连观察、标题重发现 5/5 结论、旧 token 直连 461、覆盖缺口与本文件全部既有结论维持不变；主账号不参与在线探测；E-C 冻结；离线边界不变。
