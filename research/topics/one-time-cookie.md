...
日期：2026-08-16
状态：公开资料研究 + 用户定向决定记录；任何实现与在线实验均未开始
...
5. 该路线落在项目哪个阶段、需要遵守哪些既有契约；用户本轮（2026-08-16 对话）的定向决定是什么？
...
## 8. 用户定向决定（2026-08-16 对话确认，直接引用口径）
...
- 上述决定是**用户定向决定**；正式边界修订决策记录（`docs/decisions/2026-08-16-stage4-risk-isolation.md`）尚未创建，旧文档（`product-design.md` 非目标等）尚未修改。本文档只记录决定，不替代决策记录。
- 不变项：主账号永不参与在线探测；E-C 冻结；旧 token 直连冻结；离线边界（Stage 3、`offline-input-v1`、安全清单五字段）不受任何影响。

## 9. 对项目的映射【推断】

- 位置：这条路线是 Stage 4 的 `SessionAdapter` **browser 模式**（`sync-core.md` §6.2 中未实现，进入 Stage 4 需另行授权）。
- 离线部分完全不受影响：Stage 3、`offline-input-v1`、五字段安全清单生成器均为纯本地。
...
7. 正式决策记录是否创建（`docs/decisions/2026-08-16-stage4-risk-isolation.md`）；E-C 是否解冻（当前冻结）。
...
- `projects/rednote-sync-core/docs/sync-core.md`（§2.2 阶段三禁止项；§6.2 SessionAdapter；§9.1 Stage 4 门）
