...
test("CLI rejects duplicate, extra, flag-equals, missing, positional, all, and invalid limit forms", () => {
  const invalid = [
    ...
    ["sync", "--adapter", "fixture", "--input", "page.json", ...ordinary, "--limit", "9007199254740992"],
...
test("skip reason uses the 1..80 Unicode safe-text contract", () => {
  ...
  for (const reason of ["https://fixture.invalid/n", "token=value", "has&join", "has%escape", "abc1234567890123456"]) {
    assert.equal((parseCliArgs(["task", "skip", ...ordinary, "--note-id", "note", "--reason", reason]) as Extract<CliRequest, { command: "skip" }>).reason, reason);
  }
  for (const reason of ["", "界".repeat(81), "has\ncontrol"]) {
