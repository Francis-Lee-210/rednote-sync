...
test("source ranks survive serialization and still validate their values and claimed digest", () => {
  ...
  assert.equal(isValidatedSourceRank(plain), true);
  assert.doesNotThrow(() => assertValidatedSourceRank(plain));
  assert.equal(isValidatedSourceRank({ ...rank }), true);
  assert.equal(isValidatedSourceRank(structuredClone(rank)), true);
  assert.equal(isValidatedSourceRank({ ...rank, payloadSha256: "invalid" }), false);
...
test("dynamic secrets and canaries never reach logger, reason, or persistence", () => {
  ...
  assert.doesNotThrow(() => assertPersistenceSafe("https://example.invalid/article?id=123#section2", registry));
  assert.equal(validateSafeReason("参考 id=123", registry), "参考 id=123");
  assert.throws(() => validateSafeReason("session-9-secret", registry), SafeError);
  const lines: string[] = [];
...
test("serialized secret scanning cannot be bypassed by chunk boundaries", async () => {
  ...
  try {
    ...
    const ref = await store.put(tokenSplit(), null, new AbortController().signal);
    assert.equal(await store.verify(ref), true, "a long identifier is ordinary content unless registered as a secret");
  } finally { await rm(root, { recursive: true, force: true }); }
});

test("ordinary URLs, checksums and media metadata are preserved by export and storage", async () => {
  const { renderNoteJsonBytes, renderNoteMarkdownBytes } = await import("../src/exporters.ts");
  const { FileContentAddressedObjectStore, FileMediaStore } = await import("../src/object-store.ts");
  const { readFile } = await import("node:fs/promises");
  const body = "校验 SHA-1: 0123456789abcdef0123456789abcdef01234567\n参考 https://example.com/article?id=123#section2";
  const note = makeNote("owner", "ordinary-post", time1, body);
  assert.equal(JSON.parse(Buffer.from(renderNoteJsonBytes(note)).toString()).body, body);
  assert.ok(Buffer.from(renderNoteMarkdownBytes(note)).toString().includes(body));
  const root = await tempRoot("rednote-content-fidelity-");
  try {
    const gif = await readFile(new URL("../examples/offline-v1/media/synthetic.gif", import.meta.url));
    const comment = Buffer.from("0123456789abcdef0123456789abcdef01234567");
    ...
    const stored = await new FileMediaStore(objects).put({ slot: { kind: "image", ordinal: 1 }, suggestedMimeType: "image/gif", async open() { return new ReadableStream({ start(controller) { controller.enqueue(bytes); controller.close(); } }); } }, new AbortController().signal);
    assert.deepEqual(Buffer.from(await objects.readBytes(stored.object!)), bytes);
  } finally { await rm(root, { recursive: true, force: true }); }
});
