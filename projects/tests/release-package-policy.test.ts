...
test("release package policy allows engineering changes while preserving install constraints", async () => {
  const baseline = await sourcePackage();
  assert.equal(assertReleasePackagePolicy(baseline), baseline);
  const changed = structuredClone(baseline) as {
  ...
  changed.scripts.check = "npm run typecheck";
  changed.scripts.lint = "node scripts/lint.mjs";
  changed.description = "An offline archive engine";
  changed.devDependencies.typescript = "6.0.2";
  changed.files = ["dist/", "docs/offline-input-v1.md"];
  assert.doesNotThrow(() => assertReleasePackagePolicy(changed));
  changed.dependencies = { networkClient: "1.0.0" };
  assert.throws(() => assertReleasePackagePolicy(changed), /runtime dependencies must remain empty/u);
  changed.dependencies = {};
  changed.devDependencies.typescript = "^6.0.2";
  assert.throws(() => assertReleasePackagePolicy(changed), /development dependencies must use fixed versions/u);
});
