...
import { MemorySecretRegistry, assertPersistenceSafe, assertKnownSecretBytes } from "./secrets.js";
import { decodeSha256 } from "./types.js";
const MP4_BRANDS = new Set(["isom", "iso2", "mp41", "mp42", "avc1", "M4V ", "dash"]);
export function detectMagicType(bytes, totalByteLength = bytes.byteLength) {
    const b = bytes;
    if (b.length >= 8 && Buffer.from(b.subarray(0, 8)).equals(Buffer.from([0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a])))
        return { mimeType: "image/png", extension: "png" };
    if (b.length >= 3 && b[0] === 0xff && b[1] === 0xd8 && b[2] === 0xff)
        return { mimeType: "image/jpeg", extension: "jpg" };
    if (b.length >= 6 && (Buffer.from(b.subarray(0, 6)).toString("ascii") === "GIF87a" || Buffer.from(b.subarray(0, 6)).toString("ascii") === "GIF89a"))
        return { mimeType: "image/gif", extension: "gif" };
    if (b.length >= 12 && Buffer.from(b.subarray(0, 4)).toString("ascii") === "RIFF" && Buffer.from(b.subarray(8, 12)).toString("ascii") === "WEBP")
    ...
    if (b.length >= 16 && Buffer.from(b.subarray(4, 8)).toString("ascii") === "ftyp") {
        ...
        if (boxLength >= 16 && boxLength <= totalByteLength) {
            ...
            for (let offset = 16; offset + 4 <= Math.min(boxLength, b.length); offset += 4)
                brands.push(Buffer.from(b.subarray(offset, offset + 4)).toString("ascii"));
...
async function writeAll(handle, chunk) {
    ...
    while (offset < chunk.byteLength) {
        ...
        if (result.bytesWritten <= 0)
            throw new SafeError("CANONICAL_INTEGRITY", "object write failed");
...
function sniffType(prefix, byteLength) {
    const raw = detectMagicType(prefix, byteLength);
    if (byteLength <= prefix.byteLength || !raw.mimeType.startsWith("text/") && raw.mimeType !== "application/octet-stream")
        return raw;
    ...
    if (detected.mimeType.startsWith("text/") && /^[\s]*[\[{]/.test(text))
        return { mimeType: "application/json", extension: "json" };
    return detected.mimeType.startsWith("text/") || detected.mimeType === "application/json" ? detected : raw;
}
export function objectExtension(ref) { return MIME_EXTENSIONS[ref.mimeType] ?? "bin"; }
...
export class FileContentAddressedObjectStore {
    ...
    #initialized = null;
    #issued = new WeakSet();
    constructor(root, registry = new MemorySecretRegistry()) { this.root = root; this.registry = registry; }
    owns(ref) { return this.#issued.has(ref); }
    async initialize() {
        if (!this.#initialized)
            this.#initialized = (async () => {
                ...
                for (const candidate of [path.join(trusted, "objects"), path.join(trusted, "objects", "sha256")]) {
                    await mkdir(candidate, { mode: 0o700 }).catch((error) => { if (error.code !== "EEXIST")
                        throw error; });
                    const info = await lstat(candidate);
                    if (info.isSymbolicLink() || !info.isDirectory() || info.uid !== process.getuid?.() || (info.mode & 0o777) !== 0o700)
                        throw new SafeError("SECURITY_BOUNDARY", "unsafe object directory");
                }
    ...
    objectPath(hash) { const digest = decodeSha256(hash); return path.join(this.root, "objects", "sha256", digest.slice(0, 2), digest); }
    async put(bytes, suggestedMimeType, signal) {
        if (signal.aborted)
            throw new SafeError("STATE", "object write aborted");
        await this.initialize();
        if (suggestedMimeType !== null)
            assertPersistenceSafe(suggestedMimeType, this.registry);
        const staging = path.join(this.root, "objects", "sha256", ".staging");
        await mkdir(staging, { mode: 0o700 }).catch((error) => { if (error.code !== "EEXIST")
            throw error; });
        const info = await lstat(staging);
        if (info.isSymbolicLink() || !info.isDirectory() || info.uid !== process.getuid?.() || (info.mode & 0o777) !== 0o700)
            throw new SafeError("SECURITY_BOUNDARY", "unsafe object staging directory");
        ...
        try {
            ...
            const ref = Object.freeze({ sha256: digest, byteLength, mimeType: sniffType(prefix, byteLength).mimeType });
            ...
            if (bucket.isSymbolicLink() || !bucket.isDirectory() || bucket.uid !== process.getuid?.() || (bucket.mode & 0o777) !== 0o700)
                throw new SafeError("SECURITY_BOUNDARY", "unsafe object bucket");
            await chmod(temp, 0o400);
            try {
                await link(temp, this.objectPath(digest));
                ...
                if (!await this.verify(ref))
                    throw new SafeError("CANONICAL_INTEGRITY", "existing object failed verification");
            }
            this.#issued.add(ref);
            return ref;
        }
        catch (error) {
            throw normalizeSafeError(error, "CANONICAL_INTEGRITY", "object write failed");
        }
        finally {
    ...
    async #openFile(ref) {
        ...
        for (const candidate of [this.root, path.join(this.root, "objects"), path.join(this.root, "objects", "sha256"), path.dirname(target)]) {
            ...
            if (info.isSymbolicLink() || !info.isDirectory() || info.uid !== process.getuid?.() || (info.mode & 0o777) !== 0o700)
                throw new SafeError("CANONICAL_INTEGRITY", "unsafe object directory");
        }
        const handle = await open(target, constants.O_RDONLY | constants.O_NOFOLLOW);
        try {
            const info = await handle.stat();
            if (!info.isFile() || info.uid !== process.getuid?.() || info.size !== ref.byteLength)
                throw new SafeError("CANONICAL_INTEGRITY", "object metadata mismatch");
            return handle;
        }
        catch (error) {
            await handle.close();
        ...
        return new ReadableStream({
            async pull(controller) {
                try {
                    ...
                    if (bytesRead === 0) {
                        await close();
                        if (count !== ref.byteLength)
                            throw new SafeError("CANONICAL_INTEGRITY", "object length changed");
                        controller.close();
                        return;
                    }
                    count += bytesRead;
    ...
    async verify(ref) {
        try {
            ...
            for await (const chunk of readableStreamToAsync(await this.open(ref))) {
                ...
                assertKnownSecretBytes(scan, this.registry);
    ...
    verifySync(ref) {
        try {
            const bytes = this.readBytesSync(ref);
            return sha256Bytes(bytes) === ref.sha256 && sniffType(bytes.subarray(0, SNIFF_BYTES), bytes.byteLength).mimeType === ref.mimeType;
        }
        catch {
            return false;
        }
...
export class FileMediaStore {
    ...
    async put(source, signal) {
        if (!Number.isSafeInteger(source.slot.ordinal) || source.slot.ordinal <= 0 || !["cover", "image", "video"].includes(source.slot.kind))
            throw new SafeError("INVALID_INPUT", "invalid media slot");
        ...
        if (!["png", "jpg", "gif", "webp", "mp4"].includes(extension))
            throw new SafeError("INVALID_INPUT", "media source has unsupported magic bytes");
    ...
    async verify(media) { return media.status === "stored" && media.object !== null && media.extension === objectExtension(media.object) && await this.objects.verify(media.object); }
}
