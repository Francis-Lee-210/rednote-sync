...
function requireCapability(condition, message) {
    if (!condition)
        throw new SafeError("STATE", message);
}
function sqliteCheck() {
    ...
    try {
        ...
        const row = database.prepare("SELECT count(*) AS n FROM doctor_probe").get();
...
export async function runDoctor() {
    ...
    try {
        ...
        try {
            ...
            symlinkNofollow = error.code === "ELOOP";
        }
        ...
        if (probeRoot !== null) {
            try {
                ...
                cleanup = await lstat(probeRoot).then(() => false, (error) => error.code === "ENOENT");
            }
            catch {
