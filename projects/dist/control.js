...
async function readPauseFile(target, expected) {
    ...
    try {
        ...
        if (before.isSymbolicLink() || !before.isFile() || before.uid !== process.getuid?.() || (before.mode & 0o777) !== 0o600 || before.nlink !== 1)
            throw new SafeError("SECURITY_BOUNDARY", "unsafe pause control file");
        handle = await open(target, constants.O_RDONLY | constants.O_NOFOLLOW);
        const after = await handle.stat();
        if (!after.isFile() || before.dev !== after.dev || before.ino !== after.ino)
            throw new SafeError("SECURITY_BOUNDARY", "pause control identity changed");
        const parsed = JSON.parse((await handle.readFile()).toString("utf8"));
        if (parsed === null || typeof parsed !== "object" || Array.isArray(parsed))
            throw new SafeError("INVALID_INPUT", "invalid pause control");
        const record = parsed;
        if (Object.keys(record).sort().join(",") !== "schemaVersion,scope" || record.schemaVersion !== 1)
            throw new SafeError("INVALID_INPUT", "invalid pause control");
        const scope = decodePartitionScope(record.scope);
        if (!sameScope(scope, expected))
            throw new SafeError("SECURITY_BOUNDARY", "pause control scope mismatch");
        return true;
    }
    catch (error) {
...
export async function isScopePaused(root, scopeInput) {
    ...
    let target;
    try {
        target = await resolveUnderRoot(root, pauseRelativePath(scope), false);
    }
    catch (error) {
        if (error.code === "ENOENT")
            return false;
...
export async function requestScopePause(root, scopeInput, registry = new MemorySecretRegistry()) {
    ...
    try {
        ...
        if (error.code === "EEXIST") {
            if (!await readPauseFile(target, scope))
                throw new SafeError("STATE", "pause control disappeared");
            return;
        }
...
export async function resumeScope(root, scopeInput) {
    ...
    try {
        ...
        if (error.code !== "ENOENT")
            throw normalizeSafeError(error, "STATE", "resume failed");
    }
}
