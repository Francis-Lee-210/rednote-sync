...
async function openOrdinaryFile(root, relative) {
    const resolvedRoot = path.resolve(root);
    const rootInfo = await lstat(resolvedRoot);
    if (rootInfo.isSymbolicLink() || !rootInfo.isDirectory() || rootInfo.uid !== process.getuid?.() || (rootInfo.mode & 0o777) !== 0o700)
        throw new SafeError("SECURITY_BOUNDARY", "unsafe verify root");
    const target = path.resolve(root, relative);
    if (!target.startsWith(`${resolvedRoot}${path.sep}`))
        throw new SafeError("SECURITY_BOUNDARY", "verify path escaped root");
    let cursor = resolvedRoot;
    for (const part of relative.split("/").slice(0, -1)) {
        cursor = path.join(cursor, part);
        const info = await lstat(cursor);
        if (info.isSymbolicLink() || !info.isDirectory() || info.uid !== process.getuid?.() || (info.mode & 0o777) !== 0o700)
            throw new SafeError("SECURITY_BOUNDARY", "unsafe verify ancestor");
    }
    const before = await lstat(target);
    if (before.isSymbolicLink() || !before.isFile() || before.uid !== process.getuid?.() || (before.mode & 0o777) !== 0o600 || before.nlink !== 1)
        throw new SafeError("DERIVED_VIEW", "derived receipt is missing or unsafe");
    const handle = await open(target, constants.O_RDONLY | constants.O_NOFOLLOW);
    try {
        const after = await handle.stat();
        if (!after.isFile() || before.dev !== after.dev || before.ino !== after.ino)
            throw new SafeError("DERIVED_VIEW", "derived receipt identity changed");
        return handle;
    }
    catch (error) {
        await handle.close();
...
async function verifyReceipt(root, receipt) {
    try {
        ...
        if (error instanceof SafeError && error.code === "SECURITY_BOUNDARY")
            throw error;
        return false;
    }
}
async function verifyView(root, generation, view) {
    if (!view.complete || view.generation !== generation.canonicalGeneration || view.accountKey !== generation.accountKey)
        return false;
    for (const receipt of view.receipts)
        if (!await verifyReceipt(root, receipt))
            return false;
    try {
...
async function generationMap(root) {
    ...
    try {
        ...
        try {
            return Object.freeze([...snapshot.enumerateAccountsWithGenerations()]);
        }
        finally {
            snapshot.close();
        }
        ...
        try {
            ...
            try {
                ...
                for (const generation of first) {
                    ...
                    for (const ref of snapshot.enumerateObjectRefs(generation.account))
                        if (!await objects.verify(ref))
                    ...
                    if (views.length !== PROJECTOR_ORDER.length)
                        throw new SafeError("CANONICAL_INTEGRITY", "account view rows are incomplete");
                    let derivedDirty = false;
                    for (const view of views) {
                        if (!view.complete) {
                            derivedDirty = true;
                            warnings.push(Object.freeze({ accountKey: generation.accountKey, projectorId: view.projectorId, code: "VIEW_INCOMPLETE" }));
                        }
                        else if (!await verifyView(root, generation, view)) {
                            derivedDirty = true;
                            warnings.push(Object.freeze({ accountKey: generation.accountKey, projectorId: view.projectorId, code: "VIEW_STALE" }));
                        }
                    }
                    if (generation.viewsDirty !== derivedDirty)
                        warnings.push(Object.freeze({ accountKey: generation.accountKey, projectorId: null, code: "VIEW_STALE" }));
                }
            }
            finally {
                snapshot.close();
