...
function parseExactHttps(value, origin) {
    if (typeof value !== "string")
        throw new SafeError("INVALID_INPUT", "public URL must be text");
    let parsed;
    try {
        parsed = new URL(value);
    }
    catch {
        throw new SafeError("INVALID_INPUT", "invalid public URL");
    }
...
function createHostAdapter(hostIdInput, webOrigin) {
    ...
    const sanitizeImportedUrl = (value, noteIdInput) => {
        ...
        if (parsed.pathname !== new URL(canonical).pathname)
            throw new SafeError("INVALID_INPUT", "public note URL does not match note id");
        return canonical;
    };
