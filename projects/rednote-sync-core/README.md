...
当前实现沿用插件逆向得到的单账号工作流，并在 Core 落地时采用了单账号简化假设。它尚不能表达列表关系所属账号与详情采集账号分离；未来概念边界见[账号身份概念模型](../../docs/design/account-identity-model.md)。
...
历史实施 Stage 3（包括 3C 离线加固与产品化）已经全部完成并通过独立终审。在当时的实施路线中，真实网站行为、登录会话、浏览器适配器、签名环境、限流和账号端到端验证被留给历史实施 Stage 4；这项旧边界不构成当前产品账号路线的禁令。当前工作状态以[产品路线图](../../docs/design/roadmap.md)为准。
