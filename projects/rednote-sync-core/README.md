...
> 重要：此包保持 `private: true`、`license: UNLICENSED`，没有发布到 npm，也未授予再分发许可。版本 `0.0.0-stage3` 是阶段检查点，不代表稳定版或正式发布。只支持 Node.js 26 和 macOS；Windows、Linux、其他 Node 主版本均会被安装元数据和 CLI 运行时门禁拒绝。阶段四的浏览器登录、Cookie、签名、真实 API 和在线验收尚未开始，也不得由这里的命令隐式启用。
...
以下命令从 `projects/rednote-sync-core/` Core 项目目录运行，只使用已提交的合成数据。示例中的 ID 和规范 URL 都是人为构造的测试值；`synthetic.gif` 只是以 `GIF89a` 开头、用于触发 MIME 检测的合成字节，不是可展示图片。
...
`--minimum-interval-ms 0` 仅用于无网络合成演示，让连续调用无需等待；省略时默认最小间隔为十分钟。该参数不提供在线适配器。

## 安全边界和运行要求

- 运行时必须是 Node.js 26；项目的运行依赖和开发依赖均为空，不需要 `npm install`。
...
- 所有 Stage 3 命令均严格离线。代码静态检查禁止网络内置模块和 `fetch`；离线演示还在 Node permission mode 下不给子进程网络权限。
- 每次 `sync` 最多请求输入 envelope 中的一页，详情和媒体串行处理。继续分页或重放必须再次调用命令；没有后台循环或并行目标。
- 不要把 Cookie、会话、token、授权码或其他 secret 写入 envelope、参数、root 或路径。
...
Core 项目的安全门禁先用共享的 exact policy 核对 `package.json` 全部允许字段、`files` 和 `scripts` 的键及值，并在启动任何 npm 子进程前拒绝 `preinstall`、`install`、`postinstall`、`prepare`、`prepack`、`postpack`、publish/version 等 lifecycle 脚本。通过后才在物理临时目录内运行真实 `npm pack --json`，使用独立空 npm cache、offline、no-audit、no-fund；`ignore-scripts` 只是额外防线，不是拒绝 lifecycle 的依据。随后从刚生成的本地 tarball 安装到另一个临时工程并执行安装验收：

```bash
npm run pack:check
```

该命令连续执行两次 pack 并要求两个 tarball 逐字节相同。每次都直接解析 tar entry：所有路径先过禁止规则，directory、symlink 和特殊 entry 一律拒绝；tar 内 `package/package.json` 的原始字节还要再次通过同一 metadata/files/scripts policy。命令只输出一行 JSON，其中包含 tarball SHA-256、包内逐文件 SHA-256 inventory、文件数与大小；临时 tarball、cache、安装目录和同步 root 在结束时删除，不会在 Core 项目目录留下 `node_modules` 或 `.tgz`。它不发布包。
...
安全包只含 `README.md`、`package.json`、可复现生成的 `dist/*.js` 运行文件、[离线输入文档](docs/offline-input-v1.md)、[离线输入 Schema](schemas/offline-input-v1.schema.json)和完全合成的 `examples/offline-v1/`。Node 26 不允许在 `node_modules` 中直接 type-strip `.ts`，因此安装包不携带或执行 `src/*.ts`；`build:check` 会在临时目录用 Node 内置 `stripTypeScriptTypes` 重建，并由受控词法扫描只把静态 import、side-effect import、export-from 和单一普通引号字符串 dynamic import 中的相对 `.ts` specifier 改为 `.js`。普通字符串、注释、正则和 template quasi 的静态文字保持原样；`${…}` 表达式会递归扫描真实 import，但其中除注释起始符以外的裸 `/`（包括正则和除法）不属于安全 build 子集，会直接 fail-closed。变换后再逐字节核对 `dist`。安全包不含 Core 项目的 `src/`、`tests/`、`scripts/`、`docs/sync-core.md`、`schemas/offline-artifacts.json`、`tsconfig.json` 或 `.DS_Store`；工作区的原型、研究和逆向材料位于 Core 包根之外，也不会进入分发制品。
...
Core 项目开发时，`npm run check` 会先按脚本内固定规则发现离线文档、Schema 和全部样例，再与项目内部的离线产物 allowlist exact 比较；manifest 不决定读取范围。门禁逐级拒绝符号链接和 Core 项目目录逃逸，并拒绝漏扫产物、credential 字段/值、tokenized URL、非 synthetic ID 和占位符。零依赖的受控 Schema evaluator还会检查全部 JSON 样例；它只实现本说明 Schema 实际使用的2020-12关键字，不是新的运行时 decoder。检查脚本和内部 manifest 不进入本地安装包。
...
工作区另有阶段一至三的内部设计与审查材料，但这些材料不进入本地安装包，避免把逆向报告或研究材料混入分发制品。

阶段三（包括 3C 离线加固与产品化）已经全部完成并通过独立终审。当前工作暂停在阶段四之前；真实网站行为、登录会话、浏览器适配器、签名环境、限流和账号端到端验证全部属于阶段四，没有新的明确授权时不得进入。
