...
test("real SIGKILL at object/canonical/projection/finalize/commit boundaries leaves a v1→v2 note transaction all-old or all-new", async () => {
  for (const phase of ["object", "canonical", "projection", "finalize", "commit"] as const) {
    ...
    if (phase === "commit") {
      ...
      const referenced = new Set(after.refs.map((ref) => ref.sha256));
...
test("paused replay keeps the original writer lock through rewind and footprint restore", async () => {
  const { root, scope } = await seeded("rednote-pause-rewind-lock-", "pause-rewind-lock");
  const session = await openStage3bFixture(root, scope, [fixturePage(scope, null, [])], [], "fixture", "pause-rewind.json");
  let paused = false;
  ...
  const result = await new SyncEngine({
    ...
    projectionBoundaryObserver: async (point) => {
      if (!paused && point.projectorId === "notes" && point.phase === "before_projector") { paused = true; await requestScopePause(root, scope); }
    },
    projectionRecoveryObserver: (point) => {
      assert.equal(Object.isFrozen(point), true);
      if (point.phase !== "after_rewind_before_restore") return;
      const second = spawnSync(process.execPath, [path.resolve("src/cli.ts"), "repair-views", "--root", root, "--host", scope.hostId, "--account", scope.accountId], { cwd: path.resolve("."), encoding: "utf8", timeout: 10_000 });
      assert.equal(second.status, 8);
      assert.equal(JSON.parse(second.stdout.trim()).error.category, "BUSY");
      lockChecked = true;
