...
test("fixture session exposes one provenance-bound page, details, retry source, and no-follow media factories", async () => {
  ...
  await assert.rejects(session.client.listPage(expectedScope, null, 5), safeCode("STATE"));
  session.close();
...
test("read-only input capability rejects root, fixture, ancestor, and media symlinks plus path escapes", async () => {
  ...
  await assert.rejects(transient.mediaSources[0]!.open(new AbortController().signal), safeCode("SECURITY_BOUNDARY"));
...
test("import-json mode is provenance-bound and retry source requires one exact scope+note match", async () => {
  ...
  await assert.rejects(session.retrySource.get(scope() as never, "synthetic-note" as never), safeCode("INVALID_INPUT"));
