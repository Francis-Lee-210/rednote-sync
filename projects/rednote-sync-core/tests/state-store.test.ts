...
test("paused replay rewind retains the writer lock and invalidates prior transaction capabilities", async () => {
  const root = await tempRoot("rednote-rewind-capabilities-");
  try {
    await initAccount(root);
    ...
    const identity = tx.transactionIdentity;
    assert.throws(() => second.beginImmediate({ command: "repair_views", account }), (error: unknown) => error instanceof SafeError && error.code === "BUSY");
    const plan = store.prepareCanonicalWritePlan(tx, withMergeCandidates({ scope, expectedStateRevision: 0, expectedManifestRevisions: {}, nextState: state, manifests: [], tasks: [], failures: [] }));
    store.putCanonicalMutation(tx, plan);
    const business = store.prepareBusinessResult(tx, { runId: "rewind-old", outcome: "committed", startedAt: time1, finishedAt: time2, safeErrorCategory: null, listed: 0, done: 0, partial: 0, failed: 0, skipped: 0, primaryNoteIds: [] });
    const oldBatch = await projectAccountInOrder(createProjectors(root, objects), tx, account, 2, business);
    store.rewindWriteForPausedReplay(tx);
    assert.equal(tx.transactionIdentity, identity);
    assert.throws(() => second.beginImmediate({ command: "repair_views", account }), (error: unknown) => error instanceof SafeError && error.code === "BUSY", "ROLLBACK TO must preserve BEGIN IMMEDIATE");
    assert.throws(() => store.putCanonicalMutation(tx, plan), (error: unknown) => error instanceof SafeError && error.code === "STATE");
    assert.throws(() => store.finalizeAccount(tx, oldBatch), (error: unknown) => error instanceof SafeError && error.code === "STATE", "old projection batch cannot survive the epoch change");
    const replayBusiness = store.prepareBusinessResult(tx, { runId: "rewind-paused", outcome: "paused", startedAt: time1, finishedAt: time2, safeErrorCategory: "PAUSED", listed: 0, done: 0, partial: 0, failed: 0, skipped: 0, primaryNoteIds: [] });
    const replayBatch = await projectAccountInOrder(createProjectors(root, objects), tx, account, 2, replayBusiness);
    store.finalizeAccount(tx, replayBatch);
    store.commit(tx);
    assert.throws(() => store.rewindWriteForPausedReplay(tx), (error: unknown) => error instanceof SafeError && error.code === "STATE");
    const read = store.openReadSnapshot();
    assert.equal(read.loadState(scope), null);
    assert.equal([...read.enumerateRuns(account)].at(-1)?.outcome, "paused");
    read.close(); second.close(); store.close();
  } finally { await rm(root, { recursive: true, force: true }); }
});
...
test("canonical mutation, all projectors, final run, and formal snapshot enumerators are atomic", async () => {
  ...
  try {
    ...
    assert.equal(read.loadManifest(partition, manifest.noteId)?.contentHash, undefined);
    read.close();
    store.close();
  } finally { await rm(root, { recursive: true, force: true }); }
});
