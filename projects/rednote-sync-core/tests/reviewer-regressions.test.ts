...
test("canonical factory merges a low-rank candidate against a reopened prior without treating the final manifest as the candidate", async () => {
  ...
  try {
    ...
    assert.ok(merged.note.tags.includes("low-rank"));
    assert.equal(merged.note.metrics.liked, 99);
...
test("reopened media failure occurrences merge attempt and rank exactly and require the note candidate", async () => {
  ...
  try {
    ...
    const highRank = createValidatedNoteRank(note, time2);
    const highSlot = { ...lowSlot, sourceRank: highRank };
    const mergedHigh = mergeCanonicalNote(manifest1, { note, sourceRank: highRank, mediaSlots: [highSlot], mediaSetComplete: false });
    const manifest2: NoteManifest = Object.freeze({ ...manifest1, revision: 2, semanticSourceRank: mergedHigh.sourceRank, mediaSlots: mergedHigh.mediaSlots });
    ...
    const highRaw = { ...highRequest, candidates: [{ note, sourceRank: highRank, mediaSlots: [highSlot], mediaSetComplete: false }], failureOccurrences: [{ failure: occurrence2 }], failureResolutions: [] };
