...
test("backfill response cursor is independent from note id and reaches head", () => {
  const first = transitionSyncProgress(initialProgress(), { requestCursor: null, nextCursor: c("cursor-1"), hasMore: true, noteIds: [n("note-1")], newNoteIds: [n("note-1")], allTerminal: true });
  assert.equal(first.cursor, "cursor-1");
  assert.notEqual(first.cursor, first.headFrontier[0]);
  const end = transitionSyncProgress(first, { requestCursor: c("cursor-1"), nextCursor: null, hasMore: false, noteIds: [], newNoteIds: [], allTerminal: true });
...
test("head pinned frontier plus new page uses service_end catchup", () => {
  ...
  const next = transitionSyncProgress(head, { requestCursor: null, nextCursor: c("response-cursor"), hasMore: true, noteIds: [n("old"), n("new")], newNoteIds: [n("new")], allTerminal: true });
...
test("head new page without old frontier uses frontier stop and blocker is byte-stable", () => {
  const head = Object.freeze({ phase: "head" as const, cursor: null, cursorSource: null, reachedEnd: true, headFrontier: Object.freeze([n("old")]), catchupStop: null, pendingHeadFrontier: Object.freeze([]) });
  const next = transitionSyncProgress(head, { requestCursor: null, nextCursor: c("next"), hasMore: true, noteIds: [n("new")], newNoteIds: [n("new")], allTerminal: true });
  assert.deepEqual(next.catchupStop, { kind: "frontier", noteIds: [n("old")] });
  const blocked = transitionSyncProgress(next, { requestCursor: c("next"), nextCursor: c("later"), hasMore: true, noteIds: [n("later")], newNoteIds: [n("later")], allTerminal: false });
  assert.equal(blocked, next);
});
...
test("head empty frontier conservatively catches up and missing frontier ends at service end", () => {
  ...
  const catchup = transitionSyncProgress(emptyHead, { requestCursor: null, nextCursor: c("next"), hasMore: true, noteIds: [], newNoteIds: [], allTerminal: true });
  assert.deepEqual(catchup.catchupStop, { kind: "service_end" });
  assert.deepEqual(transitionSyncProgress(catchup, { requestCursor: c("next"), nextCursor: null, hasMore: false, noteIds: [], newNoteIds: [], allTerminal: true }), emptyHead);
});

test("continued page rejects missing or repeated response cursor", () => {
  assert.throws(() => transitionSyncProgress(initialProgress(), { requestCursor: null, nextCursor: null, hasMore: true, noteIds: [], newNoteIds: [], allTerminal: true }));
  assert.throws(() => transitionSyncProgress(initialProgress(), { requestCursor: c("same"), nextCursor: c("same"), hasMore: true, noteIds: [], newNoteIds: [], allTerminal: true }));
});
