...
import { assertFixtureSession, OfflineAdapterError, type FixtureSession, type TransientNote } from "./offline-input.ts";
import { transitionSyncProgress } from "./progress.ts";
import { createProjectionBoundary, createProjectors, isProjectionPaused, projectAccountInOrder, restoreProjectionAttempt, settleProjectionAttempt, type ProjectionBoundaryPoint, type ProjectionRecoveryActionPoint, type ProjectionSettlementPoint } from "./projectors.ts";
...
export interface SyncEngineOptions {
  ...
  readonly projectionRecoveryObserver?: (point: { readonly phase: "after_rewind_before_restore" | "after_restore" } | ProjectionRecoveryActionPoint) => void | Promise<void>;
  readonly projectionSettlementObserver?: (point: ProjectionSettlementPoint) => void | Promise<void>;
}

export interface EngineResult { readonly exitCode: number; readonly run: RunRecord }
...
function terminal(status: TaskRecord["status"]): boolean { return status === "done" || status === "partial" || status === "skipped"; }
...
async function verifiedDuplicate(tx: SqliteWriteSnapshot, objects: FileContentAddressedObjectStore, scope: PartitionScope, noteId: NoteId): Promise<TaskRecord | null> {
...
export class SyncEngine {
  ...
  readonly #itemDelayMs: number;
  readonly #registry: MemorySecretRegistry;
  readonly #projectionBoundaryObserver: ((point: ProjectionBoundaryPoint) => void | Promise<void>) | null;
  readonly #projectionRecoveryObserver: ((point: { readonly phase: "after_rewind_before_restore" | "after_restore" } | ProjectionRecoveryActionPoint) => void | Promise<void>) | null;
  readonly #projectionSettlementObserver: ((point: ProjectionSettlementPoint) => void | Promise<void>) | null;
  ...
  async #seal(store: SqliteStateStore, objects: FileContentAddressedObjectStore, tx: SqliteWriteSnapshot, request: { readonly outcome: Exclude<RunOutcome, "committed_with_warnings">; readonly category: ErrorCategory | null; readonly startedAt: IsoDateTime; readonly finishedAt: IsoDateTime; readonly primaryNoteIds: readonly NoteId[] }, signal: AbortSignal | null = null): Promise<EngineResult> {
  ...
  async #replayProjectionPause(store: SqliteStateStore, objects: FileContentAddressedObjectStore, tx: SqliteWriteSnapshot, startedAt: IsoDateTime): Promise<EngineResult> {
    const account = accountPartition(this.#scope);
  ...
  async #sealWithProjectionPauseRecovery(store: SqliteStateStore, objects: FileContentAddressedObjectStore, tx: SqliteWriteSnapshot, request: { readonly outcome: Exclude<RunOutcome, "committed_with_warnings">; readonly category: ErrorCategory | null; readonly startedAt: IsoDateTime; readonly finishedAt: IsoDateTime; readonly primaryNoteIds: readonly NoteId[] }, signal: AbortSignal, command: "sync" | "retry"): Promise<EngineResult> {
    try { return await this.#seal(store, objects, tx, request, signal); }
      ...
      try {
        ...
        await this.#projectionRecoveryObserver?.(Object.freeze({ phase: "after_rewind_before_restore" as const }));
        await restoreProjectionAttempt(error, this.#root, this.#projectionRecoveryObserver);
        await this.#projectionRecoveryObserver?.(Object.freeze({ phase: "after_restore" as const }));
  ...
  async #processItem(tx: SqliteWriteSnapshot, objects: FileContentAddressedObjectStore, mediaStore: FileMediaStore, noteId: NoteId, detail: TransientNote, existingFailures: readonly FailureRecord[], now: IsoDateTime, signal: AbortSignal): Promise<ItemResult> {
    ...
    for (const source of detail.mediaSources) {
      ...
      try {
        ...
        const media: Media = Object.freeze({ ...stored, mediaId: source.mediaId });
        ...
        const failed: Media = Object.freeze({ mediaId: source.mediaId, kind: source.slot.kind, ordinal: source.slot.ordinal, status: "failed", extension: null, object: null });
    ...
    }).map((failure) => Object.freeze({ ...failure, resolvedAt: now, resolvedReason: "repaired" as const }));
    ...
    const touchedScopes = new Map<string, PartitionScope>();
  ...
  async #commitStateOutcome(store: SqliteStateStore, objects: FileContentAddressedObjectStore, tx: SqliteWriteSnapshot, current: SyncState | AlbumState, startedAt: IsoDateTime, category: ErrorCategory, signal: AbortSignal, command: "sync" | "retry", retryAt: IsoDateTime | null = null): Promise<EngineResult> {
    ...
    const mutation: CanonicalMutation = Object.freeze({ scope: this.#scope, expectedStateRevision: current.revision, expectedManifestRevisions: Object.freeze({}), nextState, candidates: Object.freeze([]), manifests: Object.freeze([]), tasks: Object.freeze([]), failureOccurrences: Object.freeze([]), failureResolutions: Object.freeze([]), failures: Object.freeze([]) });
  ...
  async runPage(signal: AbortSignal): Promise<EngineResult> {
    ...
    try {
      ...
      const duplicates = new Map<NoteId, TaskRecord>();
      ...
      const candidates: ValidatedMergeCandidateInput[] = [];
      const manifests: NoteManifest[] = [];
      const occurrences: FailureOccurrence[] = [];
      const resolutions: FailureRecord[] = [];
      const failures: FailureRecord[] = [];
      let blocker: ErrorCategory | null = protocol ? "PROTOCOL" : null;
      let blockerRetryAt: IsoDateTime | null = null;
      ...
      const mutation: CanonicalMutation = Object.freeze({ scope: this.#scope, expectedStateRevision: current.revision, expectedManifestRevisions: Object.freeze(expectedManifestRevisions), nextState, candidates: Object.freeze(candidates), manifests: Object.freeze(manifests), tasks: Object.freeze(tasks), failureOccurrences: Object.freeze(occurrences), failureResolutions: Object.freeze(resolutions), failures: Object.freeze(failures) });
  ...
  async retryFailures(signal: AbortSignal): Promise<EngineResult> {
    const store = await SqliteStateStore.open(this.#root, this.#registry);
    const objects = store.objectStore();
    const mediaStore = new FileMediaStore(objects);
    let tx: SqliteWriteSnapshot | null = null;
