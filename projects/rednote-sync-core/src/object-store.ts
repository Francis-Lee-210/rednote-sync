...
interface ObjectStoreIdentity { readonly root: string; readonly registry: MemorySecretRegistry }
const OBJECT_STORE_IDENTITIES = new WeakMap<object, ObjectStoreIdentity>();
const OBJECT_REF_ISSUERS = new WeakMap<object, object>();
...
export class FileContentAddressedObjectStore implements ContentAddressedObjectStore {
  ...
  constructor(root: string, registry = new MemorySecretRegistry()) {
    this.root = root;
  ...
  owns(ref: ObjectRef): boolean { return OBJECT_REF_ISSUERS.get(ref) === this; }
  ...
  async initialize(): Promise<void> {
    ...
    await mkdir(objects, { mode: 0o700 }).catch((error) => { if ((error as NodeJS.ErrnoException).code !== "EEXIST") throw error; });
    await mkdir(sha, { mode: 0o700 }).catch((error) => { if ((error as NodeJS.ErrnoException).code !== "EEXIST") throw error; });
  ...
  async put(bytes: AsyncIterable<Uint8Array>, suggestedMimeType: string | null, signal: AbortSignal): Promise<ObjectRef> {
    ...
    try {
      ...
      const ref: ObjectRef = Object.freeze({ sha256: digest, byteLength, mimeType: magic.mimeType });
      ...
      try {
        ...
        if ((error as NodeJS.ErrnoException).code !== "EEXIST") throw normalizeSafeError(error, "CANONICAL_INTEGRITY", "object publish failed");
  ...
  async #readVerified(ref: ObjectRef): Promise<Uint8Array> {
...
export class FileMediaStore implements MediaStore {
  ...
  async verify(media: Media): Promise<boolean> {
