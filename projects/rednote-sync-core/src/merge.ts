...
function normalizedNullable(value: string | null): string | null { return value === null ? null : normalizedText(value).trim(); }
...
export function mergeAccount(left: Account, right: Account): Account {
  ...
  const names = [left.displayName, right.displayName].filter((name): name is string => name !== null).map((name) => name.normalize("NFC").trim()).filter(Boolean).sort(utf8Compare);
  return Object.freeze({
    schemaVersion: 1,
    accountId: left.accountId,
    hostId: left.hostId,
    displayName: names.at(-1) ?? null,
...
function mergeMetric(left: number | null, right: number | null): number | null {
...
function mergeMediaSlots(existing: readonly MediaSlotState[], candidate: readonly MediaSlotState[], candidateRank: ValidatedSourceRank, mediaSetComplete: boolean): readonly MediaSlotState[] {
  ...
  for (const raw of candidate) {
    if (compareSourceRank(raw.sourceRank, candidateRank) !== 0) throw new SafeError("INVALID_INPUT", "media rank differs from candidate rank");
    const item: MediaSlotState = Object.freeze({ ...raw, media: raw.media === null ? null : normalizeMedia(raw.media) });
    ...
    if (!old || compareSourceRank(item.sourceRank, old.sourceRank) > 0) result.set(key, item);
      ...
      else if ((item.media?.object?.sha256 ?? "") > (old.media?.object?.sha256 ?? "")) result.set(key, item);
    }
  }
  if (mediaSetComplete) {
    for (const [key, old] of result) {
      if (!seen.has(key) && compareSourceRank(candidateRank, old.sourceRank) >= 0) {
...
function normalizeNote(note: Note): Note {
  const tags = [...new Set(note.tags.map((tag) => normalizedText(tag).trim()).filter(Boolean))].sort(utf8Compare);
...
export function mergeCanonicalNote(existing: NoteManifest | null, candidate: MergeCandidate): { readonly note: Note; readonly sourceRank: SourceRank; readonly membershipNameObservations: readonly MembershipNameObservation[]; readonly mediaSlots: readonly MediaSlotState[] } {
  ...
  const mediaSlots = mergeMediaSlots(existing?.mediaSlots ?? [], candidate.mediaSlots, candidate.sourceRank, candidate.mediaSetComplete);
  if (!existing) {
  ...
  const candidateWins = compareSourceRank(candidate.sourceRank, existing.semanticSourceRank) >= 0;
  ...
  const merged = normalizeNote({
    ...
    tags: [...old.tags, ...incoming.tags],
    metrics: {
      liked: mergeMetric(old.metrics.liked, incoming.metrics.liked),
      collected: mergeMetric(old.metrics.collected, incoming.metrics.collected),
      commented: mergeMetric(old.metrics.commented, incoming.metrics.commented),
      shared: mergeMetric(old.metrics.shared, incoming.metrics.shared),
    },
    memberships: memberships.memberships,
    media,
  });
  return Object.freeze({ note: merged, sourceRank: maxSourceRank(existing.semanticSourceRank, candidate.sourceRank), membershipNameObservations: memberships.observations, mediaSlots });
}
...
export function indexEntryDigest(note: Note): Sha256 { return decodeSha256(sha256Bytes(Buffer.from(canonicalJson(publicNoteProjection(note)), "utf8"))); }
