...
const ISSUED_FIXTURE_SESSIONS = new WeakSet        ();
...
function decodeAdapterFailure(value         , allowed                                          )                      {
  const raw = exactRecord(value, ["category", "retryAt"]);
  if (typeof raw.category !== "string" || !allowed.includes(raw.category                                 )) invalidInput("invalid offline adapter failure category");
  ...
  return new OfflineAdapterError(raw.category                                 , retryAt);
}
...
function decodeDetail(value         , scope                , input                   , maxMediaBytes        )                {
  ...
  const mediaSources = raw.media.map((entry)                      => {
    ...
    if (!Number.isSafeInteger(item.ordinal) || (item.ordinal          ) < 1) invalidInput("invalid offline media ordinal");
...
function decodePage(value         , account                  , input                   , maxMediaBytes        )              {
...
function decodeRetryItem(value         , account                  , input                   , maxMediaBytes        )                   {
...
function findRetry(items                             , scope                , noteId        )                   {
...
export function assertFixtureSession(value         )                                  {
