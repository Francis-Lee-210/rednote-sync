...
function normalizedNullable(value               )                { return value === null ? null : normalizedText(value).trim(); }
...
export function mergeAccount(left         , right         )          {
  ...
  const names = [left.displayName, right.displayName].filter((name)                 => name !== null).map((name) => name.normalize("NFC").trim()).filter(Boolean).sort(utf8Compare);
...
function mergeMediaSlots(existing                           , candidate                           , candidateRank                     , mediaSetComplete         )                            {
  ...
  for (const raw of candidate) {
    ...
    const item                 = Object.freeze({ ...raw, media: raw.media === null ? null : normalizeMedia(raw.media) });
...
export function indexEntryDigest(note      )         { return decodeSha256(sha256Bytes(Buffer.from(canonicalJson(publicNoteProjection(note)), "utf8"))); }
