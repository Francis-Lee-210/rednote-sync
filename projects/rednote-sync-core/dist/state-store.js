...
import { canonicalJson, sha256Canonical, utf8Compare } from "./canonical.js";
...
import { FileContentAddressedObjectStore, detectMagicType } from "./object-store.js";
...
function parseJson   (text         )    {
  ...
  try { return JSON.parse(text)     ; } catch { throw new SafeError("CANONICAL_INTEGRITY", "invalid database JSON"); }
}
function rowString(row                         , field        )         { const v = row[field]; if (typeof v !== "string") throw new SafeError("CANONICAL_INTEGRITY", "invalid database row"); return v; }
...
function decodeViewReceipt(value         )                     {
  ...
  if (!PROJECTOR_ORDER.includes(r.projectorId               ) || typeof r.generation !== "number" || !Number.isSafeInteger(r.generation) || r.generation < 0 || typeof r.byteLength !== "number" || !Number.isSafeInteger(r.byteLength) || r.byteLength < 0) throw new SafeError("CANONICAL_INTEGRITY", "invalid view receipt");
...
export function applySequentialMigrations(db              , fromVersion        , toVersion        , steps                                    )       {
  ...
  #owner()                   { const owner = SNAPSHOT_STORES.get(this); if (!owner) throw new SafeError("STATE", "invalid snapshot capability"); return owner; }
  assertActive()       { if (!this.#active || !isExactActiveSnapshot(this)) throw new SafeError("STATE", "closed or foreign snapshot"); }
  invalidate()       { this.#active = false; SNAPSHOT_ACTIVE.delete(this); const owner = SNAPSHOT_STORES.get(this); if (owner && STORE_ACTIVE_SNAPSHOTS.get(owner) === this) STORE_ACTIVE_SNAPSHOTS.delete(owner); }
...
function storeQueries(store                  )               { const queries = STORE_QUERIES.get(store); if (!queries) throw new SafeError("STATE", "invalid store query capability"); return queries; }
function snapshotOwner(snapshot        )                   { const owner = SNAPSHOT_STORES.get(snapshot); if (!owner) throw new SafeError("STATE", "invalid snapshot capability"); return owner; }
...
function assertExactSnapshot(snapshot        )       { if (!isExactActiveSnapshot(snapshot)) throw new SafeError("STATE", "closed or foreign snapshot"); }
function writeState(snapshot        )                        { const state = WRITE_STATES.get(snapshot); if (!state) throw new SafeError("STATE", "invalid write transaction capability"); return state; }

class ReadSnapshot extends SnapshotBase {          kind = "read"         ; }
class WriteSnapshot extends SnapshotBase                                {
           kind = "write"         ;
           transactionIdentity = Symbol("sqlite-write-transaction");
           intent                ;
  constructor(store                  , intent                , issuanceToken        ) { super(store, issuanceToken); this.intent = intent; WRITE_SNAPSHOTS.add(this); WRITE_STATES.set(this, { intent, epoch: 0, rewoundForPausedReplay: false, finalized: false, mutationWritten: false, businessResultIssued: false, primaryNoteIds: intent.command === "repair_views" ? Object.freeze([]) : null, summary: intent.command === "repair_views" ? Object.freeze({ listed: 0, done: 0, partial: 0, failed: 0, skipped: 0, mutationApplied: false }) : null }); Object.freeze(this); }
}
...
export class SqliteStateStore {
  ...
  close()       {
    ...
    this.#closed = true;
          ...
          assertAvailable()       { if (this.#closed) throw new SafeError("STATE", "store is closed"); if (this.#active) throw new SafeError("STATE", "connection already has a transaction"); }
  ...
  openReadSnapshot()                     { this.assertAvailable(); try { this.#db.exec("BEGIN"); } catch (error) { throw normalizeSafeError(error, "STATE", "read snapshot failed"); } const snapshot = new ReadSnapshot(this, SNAPSHOT_ISSUANCE_TOKEN); this.#active = snapshot; STORE_ACTIVE_SNAPSHOTS.set(this, snapshot); return snapshot; }
  beginImmediate(intent                )                      {
    ...
    else { const decoded = exactCommand(raw, ["command", "scope"]); if (!['sync', 'retry', 'skip', 'ack'].includes(String(decoded.command))) throw new SafeError("INVALID_INPUT", "invalid mutation intent"); normalizedIntent = Object.freeze({ command: decoded.command                                     , scope: decodePartitionScope(decoded.scope) }); }
  ...
  rewindWriteForPausedReplay(snapshot                     )       {
  ...
  closeSnapshot(snapshot              )       { this.assertSnapshot(snapshot); try { this.#db.exec("ROLLBACK"); } catch (error) { throw normalizeSafeError(error, "STATE", "snapshot close failed"); } snapshot.invalidate(); this.#active = null; }
  ...
  loadAccount(snapshot                    , hostId        , accountId                                )                 {
    ...
    if (Number(row.byte_length) !== ref.byteLength || String(row.mime_type) !== ref.mimeType) throw new SafeError("CANONICAL_INTEGRITY", "object metadata conflict");
          ...
          auditTaskFailureRelations(account                  )       {
  ...
  #queryViews(key            )                         {
    ...
    if ([...rows.keys()].some((id) => !PROJECTOR_ORDER.includes(id               ))) throw new SafeError("CANONICAL_INTEGRITY", "unknown view projector row");
  ...
  #queryManifest(account                  , id        )                      {
    ...
    const slots = slotRows.map((slot) => {
      const decoded = decodeNoteManifest({ ...value, mediaSlots: [parseJson         (slot.record_json)] }).mediaSlots[0] ;
  ...
  #queryManifests(account                  )                 { const decoded = decodeAccountPartition(account); return asRows(this.#db.prepare("SELECT note_id FROM note_manifests WHERE account_key=?").all(accountKey(decoded))).map((row) => this.#queryManifest(decoded, decodeNoteId(row.note_id)) ).sort((a, b) => utf8Compare(a.noteId, b.noteId)); }
  #queryTasks(account                  )               { const decoded = decodeAccountPartition(account); return asRows(this.#db.prepare("SELECT record_json FROM target_tasks WHERE account_key=?").all(accountKey(decoded))).map((row) => { const task = decodeTaskRecord(parseJson         (row.record_json)); return this.#queryTask(task.scope, task.noteId) ; }).sort((a, b) => utf8Compare(canonicalJson([a.scope.hostId, a.scope.accountId, a.scope.target, a.scope.boardId, a.noteId]), canonicalJson([b.scope.hostId, b.scope.accountId, b.scope.target, b.scope.boardId, b.noteId]))); }
