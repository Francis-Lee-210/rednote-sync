...
export async function ensureSecureRoot(root        )                  {
  ...
  for (const [index, component] of components.entries()) {
    ...
    try { await lstat(current); }
      ...
      await mkdir(current, { mode: 0o700 }).catch((mkdirError) => { if ((mkdirError                         ).code !== "EEXIST") throw mkdirError; });
    }
...
export async function resolveUnderRoot(root        , relative              , createParents = false)                  {
  ...
  for (let i = 0; i < parts.length - 1; i += 1) {
    ...
    if (createParents) await mkdir(next, { mode: 0o700 }).catch((error) => { if ((error                         ).code !== "EEXIST") throw error; });
