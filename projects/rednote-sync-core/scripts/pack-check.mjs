...
import { access, chmod, lstat, mkdir, mkdtemp, open, readFile, realpath, rm, writeFile } from "node:fs/promises";
import { tmpdir } from "node:os";
import path from "node:path";
import { spawnSync } from "node:child_process";
import { assertReleasePackagePolicy, EXPECTED_FILES_ALLOWLIST, FORBIDDEN_PACKAGE_PATH } from "./release-package-policy.mjs";
import { parseReleaseTar } from "./release-tar.mjs";

const EXPECTED_PACKED_FILES = Object.freeze(["README.md", "package.json", ...EXPECTED_FILES_ALLOWLIST].sort());
...
async function main() {
  ...
  run(process.execPath, ["--disable-warning=ExperimentalWarning", "scripts/build-dist.mjs", "--check"], { cwd: projectRoot }, "reproducible dist check");
  for (const relative of packageJson.files) {
    assert.doesNotMatch(relative, FORBIDDEN_PACKAGE_PATH);
    assert.doesNotMatch(relative, /[*?{}\[\]]/u, "files allowlist cannot use globs");
  }
  ...
  for (const match of readme.matchAll(/\[[^\]]+\]\(([^)]+)\)/gu)) {
    ...
    assert.equal(EXPECTED_PACKED_FILES.includes(relative), true, "README relative link must resolve inside the package");
  }
  ...
  try {
    ...
    assert.deepEqual(inventory.map((entry) => entry.path), EXPECTED_PACKED_FILES, "tar inventory must exactly match the authorized package files");
    assert.deepEqual(packResult[0].files.map((entry) => entry.path).sort(), EXPECTED_PACKED_FILES, "npm inventory must match parsed tar inventory");
    assert.equal(packResult[0].entryCount, EXPECTED_PACKED_FILES.length);
