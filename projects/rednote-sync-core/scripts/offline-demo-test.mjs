...
const syncArgs = (name, target, mode = "fixture", account = "synthetic-demo-account") => ["sync", "--root", demoRoot, ...inputArgs(name, mode), ...scopeArgs(target, account), "--minimum-interval-ms", "0"];
...
try {
  ...
  runCli([...syncArgs("no-media-success.json", "liked"), "--limit", "1"]);
  assertDurableProgress("liked", { phase: "backfill", cursor: "synthetic-page-two", reachedEnd: false });
  runCli([...syncArgs("no-media-success.json", "liked"), "--limit", "1"]);
  ...
  runCli([...syncArgs("no-media-success.json", "liked"), "--limit", "1"]);
  assertDurableProgress("liked", { phase: "head", cursor: null, reachedEnd: true });
  assert.deepEqual(await stableSnapshot(accountRoot), beforePageReplay, "page replay must preserve stable bytes and mtimes");

  runCli(syncArgs("media-success.json", "collected"));
  ...
  runCli(syncArgs("failure.json", "posted"), 5);
  assertDurableProgress("posted", { phase: "backfill", cursor: null, reachedEnd: false });
  runCli(["retry-failures", "--root", demoRoot, ...inputArgs("retry-success.json"), ...scopeArgs("posted")]);
  assertDurableProgress("posted", { phase: "backfill", cursor: null, reachedEnd: false });

  runCli(syncArgs("import-json-success.json", "liked", "import-json", "synthetic-import-account"));
