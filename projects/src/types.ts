...
export interface SourceRank { readonly revisionAt: IsoDateTime | null; readonly payloadSha256: Sha256; readonly observedAt?: IsoDateTime }
...
function exactRecord(value: unknown, allowed: readonly string[], optional: readonly string[] = []): Record<string, unknown> {
  if (value === null || typeof value !== "object" || Array.isArray(value)) invalidInput();
  const record = value as Record<string, unknown>;
  if (allowed.some((key) => !Object.hasOwn(record, key)) || Object.keys(record).some((key) => !allowed.includes(key) && !optional.includes(key))) invalidInput("unexpected object fields");
  return record;
}
...
export function decodeTaskRecord(value: unknown): TaskRecord {
  ...
  const task: TaskRecord = { schemaVersion: 1, scope: decodePartitionScope(r.scope), noteId: decodeNoteId(r.noteId), status: r.status as TaskStatus, attemptCount: integer(r.attemptCount, "task attempts"), failureIds: Object.freeze(r.failureIds.map(String)), skipReason: nullableText(r.skipReason, "skip reason"), skippedAt: nullableIso(r.skippedAt), firstSeenAt: decodeIsoDateTime(r.firstSeenAt), updatedAt: decodeIsoDateTime(r.updatedAt), ...(r.listCollector === undefined ? {} : { listCollector: r.listCollector === null ? null : decodeAccountPartition(r.listCollector) }), ...(r.attempts === undefined ? {} : { attempts: decodeCaptureAttempts(r.attempts) }) };
  if (task.listCollector && task.listCollector.hostId !== task.scope.hostId || task.attempts?.some((attempt) => attempt.collector && attempt.collector.hostId !== task.scope.hostId)) invalidInput("collector host mismatch");
...
function decodeCaptureAttempts(value: unknown): readonly CaptureAttempt[] {
  if (!Array.isArray(value)) invalidInput("invalid capture attempts");
  ...
  return Object.freeze(value.map((entry): CaptureAttempt => {
    const r = exactRecord(entry, ["attemptId", "collector", "startedAt", "finishedAt", "outcome", "category"]);
    if (!["done", "partial", "failed", "paused"].includes(String(r.outcome)) || r.category !== null && !categories.includes(String(r.category))) invalidInput("invalid capture outcome");
    const startedAt = decodeIsoDateTime(r.startedAt);
    const finishedAt = decodeIsoDateTime(r.finishedAt);
    if (finishedAt < startedAt) invalidInput("invalid capture times");
    return Object.freeze({ attemptId: safeId(r.attemptId, "attempt id"), collector: r.collector === null ? null : decodeAccountPartition(r.collector), startedAt, finishedAt, outcome: r.outcome as CaptureAttempt["outcome"], category: r.category as ErrorCategory | null });
  }));
}
...
export function decodeRunRecord(value: unknown): RunRecord {
  ...
  const base = { runId: r.runId, outcome: r.outcome as RunOutcome, startedAt: decodeIsoDateTime(r.startedAt), finishedAt: decodeIsoDateTime(r.finishedAt), safeErrorCategory: r.safeErrorCategory === null ? null : String(r.safeErrorCategory) as ErrorCategory, listed: integer(r.listed, "listed"), done: integer(r.done, "done"), partial: integer(r.partial, "partial"), failed: integer(r.failed, "failed"), skipped: integer(r.skipped, "skipped"), ...(r.listCollector === undefined ? {} : { listCollector: r.listCollector === null ? null : decodeAccountPartition(r.listCollector) }), ...(r.contentCollector === undefined ? {} : { contentCollector: r.contentCollector === null ? null : decodeAccountPartition(r.contentCollector) }), ...(r.listAttempts === undefined ? {} : { listAttempts: decodeCaptureAttempts(r.listAttempts) }) };
  if (base.listCollector && base.listCollector.hostId !== account.hostId || base.contentCollector && base.contentCollector.hostId !== account.hostId || base.listAttempts?.some((attempt) => attempt.collector && attempt.collector.hostId !== account.hostId)) invalidInput("collector host mismatch");
