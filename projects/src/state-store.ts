...
import { FileContentAddressedObjectStore, objectExtension } from "./object-store.ts";
...
export interface SqliteReadSnapshot {
  ...
  listNoteTasks(account: AccountPartition, noteId: NoteId): readonly TaskRecord[];
  listNoteFailures(account: AccountPartition, noteId: NoteId): readonly FailureRecord[];
...
function decodeViewReceipt(value: unknown): DerivedViewReceipt {
  ...
  const receipt = Object.freeze({ projectorId: r.projectorId as ProjectorId, accountKey: decodeSha256(r.accountKey) as unknown as AccountKey, generation: r.generation, relativePath: decodeRelativePath(r.relativePath), sha256: decodeSha256(r.sha256), byteLength: r.byteLength });
...
abstract class SnapshotBase implements SqliteReadSnapshot {
  ...
  listNoteTasks(account: AccountPartition, noteId: NoteId): readonly TaskRecord[] { assertExactSnapshot(this); return storeQueries(this.#owner()).noteTasks(account, noteId); }
  listNoteFailures(account: AccountPartition, noteId: NoteId): readonly FailureRecord[] { assertExactSnapshot(this); return storeQueries(this.#owner()).noteFailures(account, noteId); }
...
interface WriteTransactionState { readonly intent: MutationIntent; finalized: boolean; mutationWritten: boolean; businessResultIssued: boolean; primaryNoteIds: readonly NoteId[] | null; summary: MutationSummary | null }
const WRITE_STATES = new WeakMap<object, WriteTransactionState>();
interface WritePlanProvenance { readonly transaction: object; readonly command: CanonicalMutation; readonly account: AccountPartition }
const WRITE_PLANS = new WeakMap<object, WritePlanProvenance>();
interface BusinessResultProvenance { readonly transaction: object; readonly result: BusinessResult; readonly summary: MutationSummary }
...
interface StoreQueries {
  ...
  noteTasks(account: AccountPartition, noteId: NoteId): TaskRecord[];
  noteFailures(account: AccountPartition, noteId: NoteId): FailureRecord[];
...
export function businessResultProvenance(value: unknown, transaction: SqliteWriteSnapshot): { readonly result: BusinessResult; readonly summary: MutationSummary } {
  ...
  if (!provenance || provenance.transaction !== transaction || provenance.result !== value || state.summary === null || provenance.summary !== state.summary) throw new SafeError("STATE", "untrusted or foreign business result");
...
export class SqliteStateStore {
  ...
  private auditTaskFailureRelations(account: AccountPartition, noteIds: readonly NoteId[]): void {
    ...
    for (const task of noteIds.flatMap((id) => this.#queryTasks(account, id))) {
      ...
      if (canonicalJson(recorded) !== canonicalJson(expected)) throw new SafeError("INVALID_INPUT", "task and unresolved failure relations disagree");
  ...
  private preflightCanonicalMutation(tx: WriteSnapshot, command: CanonicalMutation): AccountPartition {
    ...
    for (const failure of affected.flatMap((id) => this.#queryFailures(account, id))) prospectiveFailures.set(failure.failureId, failure);
    ...
    for (const task of affected.flatMap((id) => this.#queryTasks(account, id))) prospectiveTasks.set(relationKey(task.scope, task.noteId), task);
  ...
  prepareCanonicalWritePlan(snapshot: SqliteWriteSnapshot, mutation: CanonicalMutation): CanonicalWritePlan {
    ...
    for (let index = 0; index < command.manifests.length; index += 1) {
      ...
      if (sha256Bytes(jsonBytes) !== manifest.artifacts.json.sha256 || jsonBytes.byteLength !== manifest.artifacts.json.byteLength || sha256Bytes(markdownBytes) !== manifest.artifacts.markdown.sha256 || markdownBytes.byteLength !== manifest.artifacts.markdown.byteLength) throw new SafeError("CANONICAL_INTEGRITY", "note artifacts do not match canonical note");
      for (const slot of manifest.mediaSlots) if (slot.media?.status === "stored" && slot.media.object && slot.media.extension !== objectExtension(slot.media.object)) throw new SafeError("CANONICAL_INTEGRITY", "media extension does not match canonical MIME");
    }
  ...
  putCanonicalMutation(snapshot: SqliteWriteSnapshot, plan: CanonicalWritePlan): void {
    ...
    if (!provenance || provenance.transaction !== tx) throw new SafeError("STATE", "untrusted or foreign canonical write plan");
  ...
  commitCanonical(snapshot: SqliteWriteSnapshot, run?: RunRecord): number {
    const tx = this.assertWrite(snapshot);
    const state = writeState(tx);
    const account = state.intent.command === "repair_views" ? state.intent.account : accountPartition(state.intent.scope);
    if (!this.loadAccount(tx, account.hostId, account.accountId)) throw new SafeError("STATE", "account must exist before commit");
    if (run) { const decoded = decodeRunRecord(run); if (!sameAccount(decoded.account, account)) throw new SafeError("INVALID_INPUT", "run account mismatch"); this.#writeRun(decoded); }
  ...
  recordRun(input: RunRecord): void {
    this.assertAvailable();
  ...
  markProjectionPending(account: AccountPartition): void {
    this.assertAvailable();
    this.safeRun("UPDATE account_generations SET views_dirty=1 WHERE account_key=?", accountKey(decodeAccountPartition(account)));
  }

  recordProjection(account: AccountPartition, generation: number, outcomes: readonly ProjectionOutcome[]): boolean {
    this.assertAvailable();
    this.#db.exec("BEGIN IMMEDIATE");
    try {
      ...
      if (outcomes.length !== PROJECTOR_ORDER.length || outcomes.some((outcome, index) => outcome.projectorId !== PROJECTOR_ORDER[index] || outcome.accountKey !== key || outcome.generation !== generation)) throw new SafeError("STATE", "projection result mismatch");
      for (const outcome of outcomes) this.safeRun("INSERT OR REPLACE INTO view_generations(account_key,projector_id,generation,complete,receipts_json,safe_error) VALUES(?,?,?,?,?,?)", key, outcome.projectorId, generation, outcome.complete ? 1 : 0, this.safeJson(outcome.receipts ?? []), outcome.safeError);
  ...
  #queryAccounts(): AccountGenerationRecord[] {
    return asRows(this.#db.prepare("SELECT a.host_id,a.account_id,g.account_key,g.canonical_generation,g.views_dirty FROM account_generations g JOIN accounts a USING(account_key) ORDER BY g.account_key").all()).map((row) => {
      ...
      const key = decodeSha256(row.account_key) as unknown as AccountKey;
  ...
  #decodeManifestRow(row: Record<string, unknown>, decodedAccount: AccountPartition, decodedId: NoteId, slotRows: Record<string, unknown>[]): NoteManifest {
    const key = noteKey(decodedAccount, decodedId);
    ...
    const slots = slotRows.map((slot) => {
      const decoded = decodeMediaSlotState(parseJson<unknown>(slot.record_json));
  ...
  #queryManifests(account: AccountPartition): NoteManifest[] {
    const decoded = decodeAccountPartition(account);
    ...
    for (const row of asRows(this.#db.prepare("SELECT ms.* FROM media_slots ms JOIN note_manifests n USING(note_key) WHERE n.account_key=? ORDER BY ms.kind,ms.ordinal").all(key))) {
      const values = slots.get(String(row.note_key)) ?? []; values.push(row); slots.set(String(row.note_key), values);
    }
    return rows.map((row) => this.#decodeManifestRow(row, decoded, decodeNoteId(row.note_id), slots.get(String(row.note_key)) ?? []));
  }
  #queryTasks(account: AccountPartition, noteId?: NoteId): TaskRecord[] {
    const decoded = decodeAccountPartition(account);
    const key = accountKey(decoded);
    ...
    for (const row of asRows(this.#db.prepare(`SELECT f.scope_key,f.note_key,f.failure_id FROM task_failures f JOIN target_tasks t USING(scope_key,note_key) WHERE ${clause} ORDER BY f.failure_id`).all(...params))) {
      const relation = `${row.scope_key}:${row.note_key}`; const ids = relations.get(relation) ?? []; ids.push(String(row.failure_id)); relations.set(relation, ids);
    }
    return rows.map((row) => {
      const value = decodeTaskRecord(parseJson<unknown>(row.record_json));
