...
export type CliRequest =
  ...
  | ({ readonly command: "pause" } & ScopedRequest)
  | ({ readonly command: "resume" } & ScopedRequest)
...
function limitFrom(options: ParsedOptions): number {
  ...
  if (!/^[1-9][0-9]*$/u.test(raw) || !Number.isSafeInteger(Number(raw))) fail("limit must be a positive integer");
  return Number(raw);
}
...
export const defaultCliDependencies = Object.freeze<CliDependencies>({
