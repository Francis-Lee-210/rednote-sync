...
import { decodeRelativePath, resolveUnderRoot } from "./paths.ts";
import { MemorySecretRegistry, assertSerializedPersistenceSafe } from "./secrets.ts";
import { decodePartitionScope, sameScope, scopeKey, type PartitionScope, type RelativePath } from "./types.ts";

function pauseRelativePath(scope: PartitionScope): RelativePath {
  return decodeRelativePath(`control/${scopeKey(scope)}.pause`);
}

function pauseBytes(scope: PartitionScope, registry: MemorySecretRegistry): Uint8Array {
  const bytes = Buffer.from(`${canonicalJson({ schemaVersion: 1, scope })}\n`, "utf8");
  assertSerializedPersistenceSafe(bytes, registry);
  return bytes;
}

async function readPauseFile(target: string, expected: PartitionScope): Promise<boolean> {
  let handle;
  try {
...
export async function isScopePaused(root: string, scopeInput: PartitionScope): Promise<boolean> {
...
export async function requestScopePause(root: string, scopeInput: PartitionScope, registry = new MemorySecretRegistry()): Promise<void> {
  const scope = decodePartitionScope(scopeInput);
  ...
  try {
    ...
    if ((error as NodeJS.ErrnoException).code === "EEXIST") { if (!await readPauseFile(target, scope)) throw new SafeError("STATE", "pause control disappeared"); return; }
    throw normalizeSafeError(error, "STATE", "pause request failed");
  } finally { await handle?.close().catch(() => {}); }
}

export async function resumeScope(root: string, scopeInput: PartitionScope): Promise<void> {
  const scope = decodePartitionScope(scopeInput);
  let target: string;
  try { target = await resolveUnderRoot(root, pauseRelativePath(scope), false); }
  ...
  catch (error) { if ((error as NodeJS.ErrnoException).code !== "ENOENT") throw normalizeSafeError(error, "STATE", "resume failed"); }
}
