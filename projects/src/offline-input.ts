...
export interface OfflineListPage {
  ...
  readonly listCollector?: AccountPartition | null;
}
...
export interface TransientNote {
  ...
  readonly contentCollector?: AccountPartition | null;
}

export interface DetailRequest { readonly detail: boolean; readonly mediaSlots: readonly import("./types.ts").MediaSlot[]; readonly signal?: AbortSignal }

export interface RednoteClient {
  readonly account: AccountPartition;
  listPage(scope: PartitionScope, requestCursor: ServerCursor | null, limit: number, signal?: AbortSignal): Promise<OfflineListPage>;
  getDetail(scope: PartitionScope, noteId: NoteId, needed?: DetailRequest): Promise<TransientNote>;
}

export interface RetrySource {
  readonly account: AccountPartition;
  get(scope: PartitionScope, noteId: NoteId, needed?: DetailRequest): Promise<TransientNote>;
}

/** Source account names the archive owner; collector identities are independent. */
export interface SyncSource {
  ...
  readonly listCollector?: AccountPartition | null;
  readonly contentCollector?: AccountPartition | null;
  hasListPage?(scope: PartitionScope, requestCursor: ServerCursor | null): boolean;
}
...
export class OfflineAdapterError extends Error {
  ...
  readonly collector: AccountPartition | null | undefined;

  constructor(category: OfflineAdapterFailureCategory, retryAt: ReturnType<typeof decodeIsoDateTime> | null, collector?: AccountPartition | null) {
...
export class ReadOnlyInputRoot {
  ...
  async read(relativeInput: unknown, signal: AbortSignal, maxBytes: number): Promise<Uint8Array> {
    ...
    for await (const chunk of this.stream(relativeInput, signal, maxBytes)) chunks.push(chunk);
    return Buffer.concat(chunks);
  }

  async *stream(relativeInput: unknown, signal: AbortSignal, maxBytes: number): AsyncIterable<Uint8Array> {
...
function exactRecord(value: unknown, fields: readonly string[], optional: readonly string[] = []): Record<string, unknown> {
  if (value === null || typeof value !== "object" || Array.isArray(value)) invalidInput("invalid offline input object");
  const record = value as Record<string, unknown>;
  if (fields.some((field) => !Object.hasOwn(record, field)) || Object.keys(record).some((field) => !fields.includes(field) && !optional.includes(field))) invalidInput("unexpected offline input fields");
  return record;
}

function decodeCollector(value: unknown, owner: AccountPartition): AccountPartition | null {
  if (value === undefined || value === null) return null;
  const collector = decodeAccountPartition(value);
  if (collector.hostId !== owner.hostId) invalidInput("collector host mismatch");
  return collector;
}
...
function decodeAdapterFailure(value: unknown, allowed: readonly OfflineAdapterFailureCategory[], collector?: AccountPartition | null): OfflineAdapterError {
  ...
  return new OfflineAdapterError(raw.category as OfflineAdapterFailureCategory, retryAt, collector);
}
...
function decodeDetail(value: unknown, scope: PartitionScope, input: ReadOnlyInputRoot, maxMediaBytes: number, collector: AccountPartition | null): DecodedDetail {
...
function decodePage(value: unknown, account: AccountPartition, input: ReadOnlyInputRoot, maxMediaBytes: number, listCollector: AccountPartition | null, contentCollector: AccountPartition | null): DecodedPage {
  ...
  if (typeof raw.hasMore !== "boolean" || !Array.isArray(raw.items) || !Array.isArray(raw.details)) invalidInput("invalid offline page");
...
function decodeRetryItem(value: unknown, account: AccountPartition, input: ReadOnlyInputRoot, maxMediaBytes: number, contentCollector: AccountPartition | null): DecodedRetryItem {
...
export class FixtureSession implements SyncSource {
  readonly account: Account;
  readonly client: RednoteClient;
  readonly retrySource: RetrySource;
  readonly mode: OfflineInputMode;
  ...
  readonly listCollector: AccountPartition | null;
  readonly contentCollector: AccountPartition | null;
  readonly hasListPage: (scope: PartitionScope, requestCursor: ServerCursor | null) => boolean;
  #closed = false;
  #listCalls = 0;
  ...
  constructor(token: symbol, envelope: DecodedEnvelope) {
    ...
    const listPage = async (scopeInput: PartitionScope, cursorInput: ServerCursor | null, limit: number): Promise<OfflineListPage> => {
      ...
      if (!Number.isSafeInteger(limit) || limit < 1) invalidInput("list limit must be positive");
    ...
    const findDetail = (scope: PartitionScope, noteId: NoteId): DecodedDetail | OfflineAdapterError => {
      const retry = envelope.retryItems.find((item) => sameScope(item.scope, scope) && item.noteId === noteId);
      if (retry) return retry.detail;
      if (this.#activePage && sameScope(this.#activePage.scope, scope) && this.#activePage.details.has(noteId)) return this.#activePage.details.get(noteId)!;
      ...
      if (matches.length === 0) throw new OfflineAdapterError("DETAIL", null);
      return matches.at(-1)!;
    };
    const getDetail = async (scopeInput: PartitionScope, noteIdInput: NoteId): Promise<TransientNote> => {
      ...
      if (!sameAccount(scope, account)) invalidInput("detail account provenance mismatch");
